...
import (
	...
	"github.com/marcos-silva-rodrigues/fs-fc-imersao18/golang/internal/events/domain"
)
...
func (r *mysqlEventRepository) FindEventByID(eventID string) (*domain.Event, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&eventIDStr, &eventName, &eventLocation, &eventOrganization,
			&eventRating, &eventDate, &eventImageURL, &eventCapacity,
			&eventPrice, &partnerID, &spotID, &spotEventID, &spotName,
			&spotStatus, &spotTicketID, &ticketID, &ticketEventID,
			&ticketSpotID, &ticketKind, &ticketPrice,
		)
		if err != nil {
